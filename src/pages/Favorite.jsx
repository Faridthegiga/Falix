...
const Favorite = () => {
  ...
  const handleReset = () => {
    ...
    // Dispatch all items for reset
    const resetIds = contents.map((content) => content.id);
    dispatch(resetList(resetIds));
  };
      ...
      <div className="w-full h-full py-10">
        <ContentWrapper>
          <div className="w-full flex justify-between items-center">
            <h1 className="text-2xl sm:text-3xl font-semibold">Favorite items</h1>
            <button
              onClick={handleReset}
              className="bg-blue-500 text-white font-medium text-lg px-6 py-2 rounded"
            >
              Reset
            </button>
          </div>
                    ...
                    <span className="w-full h-full absolute top-1 right-0">
                      <MdFavorite
                        className={`bg-[#081c22] text-3xl sm:text-4xl p-1 sm:p-2 rounded-full cursor-pointer ${
                          clickedItems[item.id] ? "text-blue-500" : ""
                        }`}
